...
class Console():        
    def ui(self):
        os.system(f'cls && title [DNG] Discord Nitro Generator  ^|  Support https://discord.gg/u4t5D7MpAx' if os.name == "nt" else "clear")
        print(center(f"""\n\n
██████╗ ███╗   ██╗ ██████╗ 
██╔══██╗████╗  ██║██╔════╝            ~ Discord Nitro Generator ~
██║  ██║██╔██╗ ██║██║  ███╗     
██║  ██║██║╚██╗██║██║   ██║ github.com/Tira-tw/nitro-generator | discord.gg/u4t5D7MpAx
██████╔╝██║ ╚████║╚██████╔╝ 
╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ \n\n
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                open('./hit.txt', 'a+').write(self.code+"\n")
                try:
                ...
                Console().printer(Fore.LIGHTRED_EX, "無效", self.code)
            elif req.status_code == 429:
                # rate = (int(req.json()['retry_after']) / 1000) + 1
                Console().printer(Fore.LIGHTBLUE_EX, "實時限制", self.code)
                # time.sleep(rate)
            else:
            ...
            Console().ui()
            threading.Lock().acquire()
            print(f"{Fore.LIGHTRED_EX} 停止 > {Fore.RESET}因鍵盤中斷而停止。")
            os.system('pause >nul')
            exit()
        except:
...
if __name__ == "__main__":
    Console().ui()
    print(" "+Fore.CYAN + str(Console().proxies_count()) + Fore.RESET + " 正在刷新中...\n\n")
    DNG = Worker()
